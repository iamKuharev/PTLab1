...
@pytest.fixture()
def correct_arguments_string_yaml() -> tuple[list[str], str]:
    return ["-p", "/home/user/file.yaml"], "/home/user/file.yaml"
...
@pytest.fixture()
def noncorrect_arguments_string_yaml() -> list[str]:
    return ["/home/user/file.yaml"]
...
def test_get_path_from_correct_arguments(
        correct_arguments_string_yaml: tuple[list[str], str]) -> None:
    path = get_path_from_arguments(correct_arguments_string_yaml[0])
    assert path == correct_arguments_string_yaml[1]


def test_get_path_from_noncorrect_arguments(
        noncorrect_arguments_string_yaml: list[str]) -> None:
    with pytest.raises(SystemExit) as e:
        get_path_from_arguments(noncorrect_arguments_string_yaml[0])
    assert e.type == SystemExit
